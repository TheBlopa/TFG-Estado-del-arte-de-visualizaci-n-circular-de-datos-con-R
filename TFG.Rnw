...
<<examples_carte, echo=FALSE, fig.align='center', fig.height=2, message=FALSE>>=
datos_ejemplos = tail(mtcars,10)
library(ggplot2)
p1<-ggplot(datos_ejemplos, aes(row.names(datos_ejemplos), mpg)) +
  geom_bar(stat = 'identity')+
  xlab("Coches") + ylab("Consumo") +
  theme_minimal() + labs(title = "Coches por consumo") +
...
p2<-data %>%
  ...
  ggplot( aes(x=date, y=value)) +
    geom_line() + xlab("Fecha")+ ylab("Valor($)") + labs(title = "Valor del bitcoin") +
...
ggplot(stockCheck, aes(x = Name, y = Value,
  ...
  geom_bar(width = 0.9, stat="identity") +
  coord_polar(theta = "y") +
  xlab("") + ylab("") + labs (fill = "Grupos") +
  ylim(c(0,112)) + labs(title = "Tiempo de Vida") +
  ...
  theme_minimal() + guides(size = FALSE, color = FALSE) +
  theme(legend.position = "bottom", plot.title = element_text(hjust = 0.5),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.line = element_blank(),
...
\clearpage
Resultando en el siguiente gr\'afico.
<<plot_br_final, echo=FALSE, fig.align='center', warning=FALSE, fig.height=6.5, fig.width=6.5>>=
@
\begin{center}
\textbf{. . .}
\end{center}
...
<<plot_ggplot_cr, echo=5:6, eval=FALSE>>=
@
\clearpage
...
<<espacios, echo=FALSE, message=FALSE>>=
empty_bar <- 8
...
<<plot_ggplot_final_cr, echo=1:3, eval=FALSE>>=
@
...
<<plot_ggplot_final_cr, echo=4:8, eval=FALSE>>=
@
Resultando en este \'ultimo gr\'afico
<<plot_ggplot_final_cr, echo=FALSE, warning=FALSE, fig.align='center', fig.height=5, fig.width=5>>==
@
\clearpage
...
<<plot_pie_basic, echo=4:6, eval=FALSE>>=
@
<<plot_pie_basic, echo=FALSE, fig.height=4.5, fig.width=4.5, fig.align='center'>>=
@
\clearpage
\subsubsection{Paquete ggplot2}
...
<<plot_plotrix_2D, echo=6:7, eval=FALSE>>=
@
...
<<plot_plotrix_2D, echo=8:13, eval=FALSE>>=
@
<<plot_plotrix_2D, echo=FALSE, fig.align='center', fig.height=4, warning=FALSE>>=
@
...
<<plot_rose_1, echo=6:10, eval=FALSE>>=
@
Dando como resultado el gr\'afico.
<<plot_rose_1, echo=FALSE, fig.align='center', fig.height=4>>=
@
\begin{center}
\textbf{. . .}
\end{center}
...
<<cuadri_circle_data>>=
datos <- data.frame(
  division = letters[1:10],
  x = row.names(tail(mtcars, 10)),
  consumo = tail(mtcars$mpg, 10),
  disp = tail(mtcars$disp, 10),
  caballos = tail(mtcars$hp, 10)
)
@
...
<<cuadri_circle_col>>=
for (i in 1:10){
  if (datos$consumo[i] > mean(datos$consumo)){
    datos$colC[i] <- "#35ab15"
  }else{
    datos$colC[i] <- "#9e1f16"
  }
  if (datos$disp[i] > mean(datos$disp)){
    datos$colD[i] <- "#35ab15"
  }else{
    datos$colD[i] <- "#9e1f16"
  }
  if (datos$caballos[i] > mean(datos$caballos)){
    datos$colCa[i] <- "#35ab15"
  }else{
    datos$colCa[i] <- "#9e1f16"
  }
}
@
...
<<cuadri_circle_prep>>=
datos = rbind(datos,c(NA,NA,"Consumo", "CC", "Caballos", "black", "black", "black"))
datos$division <- letters[1:11]
@
...
circos.initialize( factors=datos$division, xlim = c(0, 5))
circos.track(ylim = c(0, 1), track.height = 0.15, bg.col = datos$colC)
circos.track(ylim = c(0, 1), track.height = 0.15, bg.col = datos$colD)
circos.track(ylim = c(0, 1), track.height = 0.15, bg.col = datos$colCa)
circos.track(ylim = c(0, 1), track.height = 0.4, bg.border = NA)
for (i in 1:11) {
  circos.text(2.5, 0.5, datos$x[i], sector.index = datos$division[i], track.index = 4, 
              facing = 'clockwise', niceFacing = TRUE, cex = 0.7)
  circos.text(2.5, 0.5, paste0(datos$consumo[i], " (mpg)"), sector.index = datos$division[i]
              , track.index = 1, niceFacing = TRUE, cex = 0.7, col = "white", font = 2)
  circos.text(2.5, 0.5, paste0(datos$disp[i], " (cc)") , sector.index = datos$division[i]
              , track.index = 2, niceFacing = TRUE, cex = 0.7, col = "white", font = 2)
  circos.text(2.5, 0.5, paste0(datos$caballos[i] ," (hp)"), sector.index = datos$division[i]
              , track.index = 3, niceFacing = TRUE, cex = 0.7, col = "white", font = 2)
}
title("Coches por Consumo, CC y Caballos de potencia")
legend("center", inset=.02, title="Media",
       c("Mejor", "Peor"), fill=c("#35ab15","#9e1f16"), cex=0.6)
circos.clear()
@
\clearpage
...
<<cuadri_circle, echo=4, eval=FALSE>>=
@
...
<<cuadri_circle, echo=5:7, eval=FALSE>>=
@
...
<<cuadri_circle, echo=8:17, eval=FALSE>>=
@
...
@
<<plot_ggraph_sol, echo=FALSE, fig.align='center', fig.height=3.4, fig.width=3.4>>=
@
...
<<plot_col_sep_gg, echo=FALSE, fig.align='center', fig.height=5, fig.width=5, warning=FALSE>>=
@
\clearpage
...
<<ggiraph_col_sep, echo=5:10, eval=FALSE>>=
@
...
<<ggiraph_col_sep, echo=11, eval=FALSE>>=
@
\clearpage
Resultando en el siguiente gr\'afico:~\\
\vbox{
...
<<ggiraphExtra, fig.align='center', fig.height=5, fig.width=5, warning=FALSE>>=
library(ggiraphExtra)
...
legend("bottomright", inset=.02, title="CCAA",
       c("Madrid","Catalunya","Castilla-La Mancha"), fill=c("burlywood","coral","blueviolet"), cex=0.6)
@
...
<<plotgg_bu, echo=4:7, eval=FALSE>>=
@
Resultando en: 
<<plotgg_bu, echo=FALSE, fig.align='center'>>=
@
\begin{center}
\textbf{. . .}
\end{center}
...
<<plot_bu, fig.align='center', fig.height=6, fig.width=6>>=
library(ggplot2)
ggplot() + 
...
<<plot_ggraph-1, fig.align='center', fig.height=4.5, fig.width=4.5>>=
ggraph(migrafo, layout = 'circlepack') +
  geom_node_circle(aes(fill = depth)) +
