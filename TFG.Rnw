...
empty_bar <- 8
to_add <- data.frame( matrix(NA, empty_bar*nrow(table(mtcars$gear)), ncol(data)) )
colnames(to_add) <- colnames(data)
to_add$gear <- rep(rownames(table(data$gear)), each=empty_bar)
...
ggplot(datatrans , aes(x=id, y=mpg, fill= gear, label=name))+
...
polar.plot(long, pos,main="Polar Plot",lwd=3,line.col=2,
           rad.col= "lightblue", grid.col= "lightblue",start = 0, labels=datos$velocidad.Var1, label.pos = pos)
...
<<plot_col_sep_gg, echo=FALSE, fig.align='center', fig.height=4, fig.width=4, warning=FALSE>>=
@
\begin{center}
\textbf{. . .}
\end{center}
...
legend("bottomright", inset=.02, title="CCAA",
       c("Madrid","Cataluña","Castilla-LaMancha"), fill=c("burlywood","coral","blueviolet"), horiz=TRUE, cex=0.6)
@
