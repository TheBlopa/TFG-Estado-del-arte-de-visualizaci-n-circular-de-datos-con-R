...
<<plot_ring_gg1, echo=5:7, eval=FALSE>>=
@
Resultando en:
...
Primero la longitud de las columnas con las cantidad de multas segun la velocidad.
<<data_long>>=
...
pie(Porcentaje, labels = c("1","2","3","4","6","8"), border = "white", col = miPaleta)
...
ggplot(data, aes(x="", y=Porcentaje, fill=Var1)) +
  ...
  theme_void() + labs(title="Coches por Carburadores") +
  theme(legend.position="none") +
  geom_text(aes(y = ypos, label = Var1), color = "black", size=5) +
...
pie.labels(3.25, 3, angulos, data$Var1, minangle = 0.2, radius = 1.6)
...
pie3D(data$Freq, labels = data$Var1, explode = 0.1, main = "Coches por Carburadores 3D",
      col=c("#ff0000","#80ff00","#00ffff","#44bbff","#8000ff"))
dev.off()
@
...
library(ggplot2)
ggplot(data,aes(x=ï..MES,y=value,fill=observation))+
...
library(ggiraphExtra)
ggRose(data,aes(x=ï..MES,fill=observation,y=value),stat="identity",interactive=FALSE
       , palette = "Blues") +
   xlab("") +  ylab("") +  
  labs(fill='Causas') + labs(title = "Procentaje de Muertes") +
  scale_y_sqrt() +
...
p <- ggRose(data,aes(x=ï..MES,fill=observation,y=value),stat="identity",interactive=TRUE
       , palette = "Blues")
library(htmlwidgets)
...
ggBar(data, aes(x=ï..MES,fill=observation,y=value),stat="identity",polar=TRUE,palette="Blues"
  ...
  labs(fill='Causas') + labs(title = "Procentaje de Muertes") +
    ...
    theme(panel.grid.major = element_blank(),
          ...
          axis.text.y = element_blank(),
          axis.ticks = element_blank())
p <- ggBar(data, aes(x=ï..MES,fill=observation,y=value),stat="identity",polar=TRUE,palette="Blues"
...
library(ggplot2)
@
Para datos del diagrama de columnas con formato iris, y el formato de columnas apiladas se eligen los fallecidos del COVID-19 por fecha y por Comunidad Aut\'onoma.
<<data_col_sep_ggplo>>=
...
circos.clear()
legend("topright", inset=.02, title="CCAA",
       c("Madrid","Cataluña","Castilla-LaMancha"), fill=c("burlywood","coral","blueviolet"), cex=0.6)
@
...
se tiene que obtener una estructura especial proporcionada por \texttt{dendextend}. Para crear el dendograma se obtienen datos num\'ericos de la base de datos \texttt{mtcars}, como ejemplo, y se crean grupos con \texttt{hclust} mediante las distancias entre los valores num\'ericos, agrupando los veh\'iculos por disp y cyl, se establecen los colores de las ramas y nodos hoja.
